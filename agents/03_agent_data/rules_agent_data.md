...
- [Gapp & Modèles] : NE PAS modifier manuellement les relations dans les modèles. Demander au développeur d'exécuter `gapp make:crud NomModel` pour mettre à jour le code via le générateur Gapp.
...
- [Gapp Sequence] : Suite à une modification de la base de données (migration), le développeur DOIT exécuter `gapp meta:sync` AVANT `gapp make:crud NomModel`.
