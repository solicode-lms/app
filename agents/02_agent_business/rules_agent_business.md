...
- [Règle Sync Pseudo-Code] : Les algorithmes métier complexes (Workflows, Calculs) doivent être documentés dans un fichier `{NomWorkflow}_logic.md` (Pseudo-code structuré) référencé dans la PHPDoc de la méthode via `@see`. En cas de modification de la logique du code, **L'AGENT DOIT DEMANDER AU DÉVELOPPEUR** : "Souhaitez-vous synchroniser le pseudo-code de documentation ?".
