...
class BaseVilleSeeder extends Seeder
{
    ...
    {
        $filePath = base_path("modules/PkgApprenants/Database/data/villes.csv");
        ...
        while (($data = fgetcsv($csvFile)) !== false) {
            ...
            if ($row) {
                ...
                if (!empty($data["reference"])) {
                    $villeService->updateOrCreate(["reference" => $data["reference"]], $villeData);
                } else {
                    $villeService->create($villeData);
                }
            }
        }
    ...
    private function addDefaultControllerDomainFeatures(): void
    {
        ...
        $controllerName = 'VilleController';
        $controllerBaseName = 'ville';
        $domainName = 'Ville';
