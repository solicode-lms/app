...
class BaseApprenantController extends AdminController
{
    ...
    public function bulkUpdate(Request $request) {
        ...
        $request->validate([
            'apprenant_ids'   => ['required', 'array', 'min:1'],
            'fields_modifiables'               => ['required', 'array', 'min:1']
        ]);

        $ids          = $request->input('apprenant_ids', []);
        ...
        $form         = new \Modules\PkgApprenants\App\Requests\ApprenantRequest();
        ...
        foreach ($ids as $id) {
            $model = $this->apprenantService->find($id);
    ...
    {
        // $this->authorizeAction('update');
        $itemApprenant = Apprenant::findOrFail($id);


        $data = $this->service->buildFieldMeta($itemApprenant, $field);
        return response()->json(
            $data
        );
    }
    ...
    public function patchInline(Request $request, int $id)
    {
        ...
        $etag = $this->service->etag($itemApprenant);
        ...
        $updated = $this->service->applyInlinePatch($itemApprenant, $changes);
