...
<div id="apprenant-crud-show">
    ...
    @endif
            ...
            @if(
                  (auth()->user()?->can('show-realisationQcm') && $itemApprenant->realisationQcms->isNotEmpty())  
                  || auth()->user()?->can('create-realisationQcm')
                  || (auth()->user()?->can('edit-realisationQcm')  && $itemApprenant->realisationQcms->isNotEmpty() )
                  )
            <div class="col-12 col-md-12 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
                  <small class="text-muted d-block">  {{ ucfirst(__('PkgQcm::realisationQcm.plural')) }}</small>
                  <div class="pt-2">
                        @include('PkgQcm::realisationQcm._index',['isMany' => true, "edit_has_many" => false,"contextKey" => 'apprenant.show_' . $itemApprenant->id])
                  </div>
                  </div>
            </div>
            @endif
