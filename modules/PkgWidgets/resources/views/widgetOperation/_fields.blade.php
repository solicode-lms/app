...
<form 
    ...
    @if ($itemWidgetOperation->id)
        <input type="hidden" name="id" value="{{ $itemWidgetOperation->id }}">
        @method('PUT')
    @endif
    @if ($bulkEdit && !empty($widgetOperation_ids))
    ...
    <div class="row">
        <x-form-field :defined_vars="get_defined_vars()" :entity="$itemWidgetOperation" field="operation" :bulkEdit="$bulkEdit">

      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="operation" id="bulk_field_operation" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
          <label for="operation">
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemWidgetOperation" field="description" :bulkEdit="$bulkEdit">
