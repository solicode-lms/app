...
<script>
    ...
    window.crudModalManagersConfig.push({
        ...
        fieldMetaUrl: '{{ route('widgets.field.meta',  ['id' => ':id', 'field' => ':field']) }}',
        patchInlineUrl: '{{ route('widgets.patchInline',  ['id' => ':id']) }}',
        showUrl: '{{ route('widgets.show',  ['widget' => ':id']) }}',
        getEntityUrl: '{{ route("widgets.getById", ["id" => ":id"]) }}',
        storeUrl: '{{ route('widgets.store') }}', 
        updateAttributesUrl: '{{ route('widgets.updateAttributes') }}', 
        deleteUrl: '{{ route('widgets.destroy',  ['widget' => ':id']) }}', 
        canEdit: @json(Auth::user()->can('edit-widget')),
    ...
    <section id="widget-crud-table" class="content crud-table">
        <div class="container-fluid">
            <div class="card card-outline card-info " id="card_crud">
                ...
                @if(!empty($widgets_total) &&  $widgets_total > 50)
                <div class="card-header">
                ...
                @section('widget-crud-bulk-actions')
