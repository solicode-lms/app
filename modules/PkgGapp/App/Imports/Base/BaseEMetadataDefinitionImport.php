...
class BaseEMetadataDefinitionImport implements ToModel, WithHeadingRow
{
     ...
     */
    private function findExistingRecord($reference): ?EMetadataDefinition
    {
        if($reference == null) return null;
        return EMetadataDefinition::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EMetadataDefinition([
             'reference' => $reference,
             'name' => $values[1] ?? null,
             'groupe' => $values[2] ?? null,
             'type' => $values[3] ?? null,
             'scope' => $values[4] ?? null,
             'description' => $values[5] ?? null,
             'default_value' => $values[6] ?? null,
        ]);
