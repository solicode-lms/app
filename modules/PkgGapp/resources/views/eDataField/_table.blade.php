        ...
        <tbody>
            ...
            @foreach ($eDataFields_data as $eDataField)
                ...
                <tr id="eDataField-row-{{$eDataField->id}}" data-id="{{$eDataField->id}}">
                    ...
                    <td style="max-width: 5%;white-space: normal;" class=" text-truncate" data-id="{{$eDataField->id}}" data-field="displayOrder">
                            <div class="sortable-button d-flex justify-content-left align-items-center" style="height: 100%;  min-height: 26px;">
                            <i class="fas fa-th-list" title="{{ $eDataField->displayOrder }}"  data-toggle="tooltip" ></i>  
                        </div>

                    </td>
                    <td style="max-width: 15%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$eDataField->id}}" data-field="name" >
                        @include('PkgGapp::eDataField.custom.fields.name', ['entity' => $eDataField])
                    </td>
                    <td style="max-width: 15%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$eDataField->id}}" data-field="e_model_id">
                        {{  $eDataField->eModel }}

                    </td>
                    <td style="max-width: 10%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$eDataField->id}}" data-field="data_type">
                        {{ $eDataField->data_type }}

                    </td>
                    <td style="max-width: 8%;white-space: normal;" class=" text-truncate" data-id="{{$eDataField->id}}" data-field="displayInTable">
