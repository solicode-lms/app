...
class BaseRelationshipSeeder extends Seeder
{
    ...
    {
        $csvFile = fopen(base_path("modules/PkgGapp/Database/data/relationships.csv"), "r");
        $firstline = true;

        while (($data = fgetcsv($csvFile)) !== false) {
            if (!$firstline) {
                Relationship::create([
                    "source_model_id" => $data[0] ,
                    "target_model_id" => $data[1] ,
                    "type" => $data[2] ,
                    "source_field" => $data[3] ,
                    "target_field" => $data[4] ,
    ...
    private function addDefaultControllerDomainFeatures(): void
    {
        ...
        $controllerName = 'RelationshipController';
        $controllerBaseName = 'relationship';
        $domainName = 'Relationship';

        // Permissions spécifiques pour chaque type de fonctionnalité
        $featurePermissions = [
            'Édition ' => [ 'create','store','edit','update','destroy','getRelationships'],
            'Lecture' => ['index', 'show'],
