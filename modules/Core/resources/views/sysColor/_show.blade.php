...
<div id="sysColor-crud-show">
    ...
    @endif
            ...
            @if(
                  (auth()->user()?->can('show-labelProjet') && $itemSysColor->labelProjets->isNotEmpty())  
                  || auth()->user()?->can('create-labelProjet')
                  || (auth()->user()?->can('edit-labelProjet')  && $itemSysColor->labelProjets->isNotEmpty() )
                  )
            <div class="col-12 col-md-12 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
                  <small class="text-muted d-block">  {{ ucfirst(__('PkgCreationProjet::labelProjet.plural')) }}</small>
                  <div class="pt-2">
                        @include('PkgCreationProjet::labelProjet._index',['isMany' => true, "edit_has_many" => false,"contextKey" => 'sysColor.show_' . $itemSysColor->id])
                  </div>
                  </div>
            </div>
            @endif
