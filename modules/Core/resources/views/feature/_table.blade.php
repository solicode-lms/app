            ...
            @foreach ($data as $feature)
                <tr>
                    ...
                    <td class="text-center">
                        @can('show-FeatureController')
                            <a href="{{ route('features.show', $feature) }}" class="btn btn-default btn-sm">
                                <i class="far fa-eye"></i>
                            </a>
                        @endcan
                        @can('edit-FeatureController')
                            <a href="{{ route('features.edit', $feature) }}" class="btn btn-sm btn-default">
                                <i class="fas fa-pen-square"></i>
                            </a>
                        @endcan
                        @can('destroy-FeatureController')
                            <form action="{{ route('features.destroy', $feature) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger"
                                    onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce feature ?')">
