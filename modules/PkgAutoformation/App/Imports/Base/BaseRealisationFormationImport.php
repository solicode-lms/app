...
class BaseRealisationFormationImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return RealisationFormation|null
     */
    private function findExistingRecord($reference): ?RealisationFormation
    {
        if($reference == null) return null;
        return RealisationFormation::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new RealisationFormation([
             'date_debut' => $values[0] ?? null,
             'date_fin' => $values[1] ?? null,
             'reference' => $reference,
             'formation_id' => $values[3] ?? null,
             'apprenant_id' => $values[4] ?? null,
             'etat_formation_id' => $values[5] ?? null,
        ]);
