...
class BaseEtatChapitreImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatChapitre|null
     */
    private function findExistingRecord($reference): ?EtatChapitre
    {
        if($reference == null) return null;
        return EtatChapitre::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EtatChapitre([
             ...
             'reference' => $reference,
             'workflow_chapitre_id' => $values[4] ?? null,
