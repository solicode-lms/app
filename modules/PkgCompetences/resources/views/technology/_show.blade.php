      ...
      <div class="col-12 col-md-6 col-lg-6 mb-3 px-2">
          <div class="border rounded p-2 h-100">
                        <small class="text-muted d-block">{{ ucfirst(__('PkgAutoformation::formation.plural')) }}</small>
                              <!-- Valeurs many-to-many -->
        @if($itemTechnology->formations->isNotEmpty())
          <div>
            @foreach($itemTechnology->formations as $formation)
              <span class="badge badge-info mr-1">
                {{ $formation }}
              </span>
            @endforeach
      ...
      <div class="col-12 col-md-6 col-lg-6 mb-3 px-2">
          <div class="border rounded p-2 h-100">
                        <small class="text-muted d-block">{{ ucfirst(__('PkgCreationProjet::transfertCompetence.plural')) }}</small>
                              <!-- Valeurs many-to-many -->
        @if($itemTechnology->transfertCompetences->isNotEmpty())
          <div>
            @foreach($itemTechnology->transfertCompetences as $transfertCompetence)
              <span class="badge badge-info mr-1">
                {{ $transfertCompetence }}
              </span>
            @endforeach
  ...
  @endif
        ...
        <div class="card-footer">
          ...
          @can('update', $itemTechnology)
              <a href="{{ route('technologies.edit', ['technology' => $itemTechnology->id]) }}" data-id="{{$itemTechnology->id}}" class="btn btn-info ml-2 editEntity">
