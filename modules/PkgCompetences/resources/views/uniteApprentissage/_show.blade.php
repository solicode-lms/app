...
@include('PkgCompetences::uniteApprentissage.custom.fields.microCompetence',['entity' => $itemUniteApprentissage])
    ...
    @endif
            ...
            @if(
                  (auth()->user()?->can('show-chapitre') && $itemUniteApprentissage->chapitres->isNotEmpty())  
                  || auth()->user()?->can('create-chapitre')
                  || (auth()->user()?->can('edit-chapitre')  && $itemUniteApprentissage->chapitres->isNotEmpty() )
                  )
            ...
            @if(
                  (auth()->user()?->can('show-critereEvaluation') && $itemUniteApprentissage->critereEvaluations->isNotEmpty())  
                  || auth()->user()?->can('create-critereEvaluation')
                  || (auth()->user()?->can('edit-critereEvaluation')  && $itemUniteApprentissage->critereEvaluations->isNotEmpty() )
                  )
            <div class="col-12 col-md-12 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
            ...
            @if(
                  (auth()->user()?->can('show-question') && $itemUniteApprentissage->questions->isNotEmpty())  
                  || auth()->user()?->can('create-question')
                  || (auth()->user()?->can('edit-question')  && $itemUniteApprentissage->questions->isNotEmpty() )
                  )
            <div class="col-12 col-md-12 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
                  <small class="text-muted d-block">  {{ ucfirst(__('PkgQcm::question.plural')) }}</small>
                  <div class="pt-2">
                        @include('PkgQcm::question._index',['isMany' => true, "edit_has_many" => false,"contextKey" => 'uniteApprentissage.show_' . $itemUniteApprentissage->id])
                  </div>
                  </div>
            </div>
            @endif
