...
<form 
    ...
    @if ($itemCompetence->id)
        <input type="hidden" name="id" value="{{ $itemCompetence->id }}">
        @method('PUT')
    @endif
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemCompetence" field="mini_code" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-4">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="mini_code" id="bulk_field_mini_code" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
          <label for="mini_code">
