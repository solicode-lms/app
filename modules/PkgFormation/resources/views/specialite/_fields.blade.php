...
<form 
    ...
    @if ($itemSpecialite->id)
        <input type="hidden" name="id" value="{{ $itemSpecialite->id }}">
        @method('PUT')
    @endif
    @if ($bulkEdit && !empty($specialite_ids))
    ...
    <div class="row">
        <x-form-field :defined_vars="get_defined_vars()" :entity="$itemSpecialite" field="nom" :bulkEdit="$bulkEdit">
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemSpecialite" field="description" :bulkEdit="$bulkEdit">
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemSpecialite" field="formateurs" :bulkEdit="$bulkEdit">

      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="formateurs" id="bulk_field_formateurs" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
