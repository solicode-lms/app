...
class BaseTacheImport implements ToModel, WithHeadingRow
{
    /**
     ...
     * @return Tache|null
     */
    private function findExistingRecord($reference): ?Tache
    {
        if($reference == null) return null;
        return Tache::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new Tache([
             ...
             'reference' => $reference,
             'note' => $values[8] ?? null,
             'is_live_coding_task' => $values[9] ?? null,
             'phase_evaluation_id' => $values[10] ?? null,
             'chapitre_id' => $values[11] ?? null,
             'livrables' => $values[12] ?? null,
        ]);
