    ...
    <table class="table table-striped text-nowrap" style="table-layout: fixed; width: 100%;">
        <thead style="width: 100%">
            <tr>
                ...
                <x-sortable-column :sortable="true" width="4"  field="ordre" modelname="prioriteTache" label="{!!ucfirst(__('PkgCreationTache::prioriteTache.ordre'))!!}" />
                <x-sortable-column :sortable="true" width="39"  field="nom" modelname="prioriteTache" label="{!!ucfirst(__('PkgCreationTache::prioriteTache.nom'))!!}" />
                <x-sortable-column :sortable="true" width="39" field="formateur_id" modelname="prioriteTache" label="{!!ucfirst(__('PkgFormation::formateur.singular'))!!}" />
        ...
        <tbody>
            ...
            @foreach ($prioriteTaches_data as $prioriteTache)
                ...
                <tr id="prioriteTache-row-{{$prioriteTache->id}}" data-id="{{$prioriteTache->id}}">
                    ...
                    <td style="max-width: 4%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$prioriteTache->id}}" data-field="ordre">
                            <div class="sortable-button d-flex justify-content-left align-items-center" style="height: 100%;  min-height: 26px;">
                            <i class="fas fa-th-list" title="{{ $prioriteTache->ordre }}"  data-toggle="tooltip" ></i>  
                        </div>

                    </td>
                    <td style="max-width: 39%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$prioriteTache->id}}" data-field="nom">
                        {{ $prioriteTache->nom }}

                    </td>
                    <td style="max-width: 39%;white-space: normal;" class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$prioriteTache->id}}" data-field="formateur_id">
