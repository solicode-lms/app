...
<form 
    ...
    @if ($itemEtatRealisationTache->id)
        <input type="hidden" name="id" value="{{ $itemEtatRealisationTache->id }}">
        @method('PUT')
    @endif
    @if ($bulkEdit && !empty($etatRealisationTache_ids))
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemEtatRealisationTache" field="workflow_tache_id" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="workflow_tache_id" id="bulk_field_workflow_tache_id" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
