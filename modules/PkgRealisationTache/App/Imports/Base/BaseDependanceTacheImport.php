...
class BaseDependanceTacheImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return DependanceTache|null
     */
    private function findExistingRecord($reference): ?DependanceTache
    {
        if($reference == null) return null;
        return DependanceTache::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new DependanceTache([
             ...
             'type_dependance_tache_id' => $values[2] ?? null,
             'tache_cible_id' => $values[3] ?? null,
        ]);
