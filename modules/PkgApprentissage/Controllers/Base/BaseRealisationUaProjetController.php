...
class BaseRealisationUaProjetController extends AdminController
{
    ...
    public function bulkUpdate(Request $request) {
        ...
        $request->validate([
            'realisationUaProjet_ids'   => ['required', 'array', 'min:1'],
            'fields_modifiables'               => ['required', 'array', 'min:1']
        ]);

        $ids          = $request->input('realisationUaProjet_ids', []);
        ...
        $form         = new \Modules\PkgApprentissage\App\Requests\RealisationUaProjetRequest();
        ...
        foreach ($ids as $id) {
            $model = $this->realisationUaProjetService->find($id);
    ...
    /**
     * @DynamicPermissionIgnore
     * Retourne une tâche (RealisationUaProjet) par ID, en format JSON.
     */
    ...
    {
        // $this->authorizeAction('update');
        $itemRealisationUaProjet = RealisationUaProjet::findOrFail($id);


        $data = $this->service->buildFieldMeta($itemRealisationUaProjet, $field);
        return response()->json(
            $data
        );
    }
    ...
    public function patchInline(Request $request, int $id)
    {
        ...
        $etag = $this->service->etag($itemRealisationUaProjet);
        ...
        $updated = $this->service->applyInlinePatch($itemRealisationUaProjet, $changes);
