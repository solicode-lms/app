...
class BaseEtatRealisationChapitreImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatRealisationChapitre|null
     */
    private function findExistingRecord($reference): ?EtatRealisationChapitre
    {
        if($reference == null) return null;
        return EtatRealisationChapitre::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EtatRealisationChapitre([
             'ordre' => $values[0] ?? null,
             'nom' => $values[1] ?? null,
             'code' => $values[2] ?? null,
