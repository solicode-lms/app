...
class BaseRealisationMicroCompetenceImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return RealisationMicroCompetence|null
     */
    private function findExistingRecord($reference): ?RealisationMicroCompetence
    {
        if($reference == null) return null;
        return RealisationMicroCompetence::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new RealisationMicroCompetence([
             ...
             'dernier_update' => $values[10] ?? null,
             'realisation_competence_id' => $values[11] ?? null,
             'reference' => $reference,
             'lien_livrable' => $values[13] ?? null,
             'progression_ideal_cache' => $values[14] ?? null,
             'taux_rythme_cache' => $values[15] ?? null,
             'pourcentage_non_valide_cache' => $values[16] ?? null,
        ]);
