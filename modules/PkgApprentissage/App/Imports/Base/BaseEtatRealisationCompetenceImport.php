...
class BaseEtatRealisationCompetenceImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatRealisationCompetence|null
     */
    private function findExistingRecord($reference): ?EtatRealisationCompetence
    {
        if($reference == null) return null;
        return EtatRealisationCompetence::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EtatRealisationCompetence([
             'ordre' => $values[0] ?? null,
             'reference' => $reference,
             'nom' => $values[2] ?? null,
