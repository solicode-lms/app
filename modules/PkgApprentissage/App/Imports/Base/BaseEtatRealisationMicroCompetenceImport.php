...
class BaseEtatRealisationMicroCompetenceImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatRealisationMicroCompetence|null
     */
    private function findExistingRecord($reference): ?EtatRealisationMicroCompetence
    {
        if($reference == null) return null;
        return EtatRealisationMicroCompetence::where('reference', $reference)->first();
    }
