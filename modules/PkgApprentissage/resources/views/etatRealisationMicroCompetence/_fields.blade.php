...
@section('etatRealisationMicroCompetence-form')
<form 
    class="crud-form custom-form context-state container" 
    id="etatRealisationMicroCompetenceForm"
    action="{{ isset($bulkEdit) && $bulkEdit ? route('etatRealisationMicroCompetences.bulkUpdate') : ($itemEtatRealisationMicroCompetence->id ? route('etatRealisationMicroCompetences.update', $itemEtatRealisationMicroCompetence->id) : route('etatRealisationMicroCompetences.store')) }}"
    method="POST"
    novalidate > 
    ...
    @if ($bulkEdit && !empty($etatRealisationMicroCompetence_ids))
        @foreach ($etatRealisationMicroCompetence_ids as $id)
            <input type="hidden" name="etatRealisationMicroCompetence_ids[]" value="{{ $id }}">
        @endforeach
    @endif
    ...
    <div class="row">
      ...
      <div class="form-group col-12 col-md-6">
          ...
          @endif
          <label for="ordre">
            {{ ucfirst(__('PkgApprentissage::etatRealisationMicroCompetence.ordre')) }}
            <span class="text-danger">*</span>
                ...
                id="ordre"
                placeholder="{{ __('PkgApprentissage::etatRealisationMicroCompetence.ordre') }}"
                value="{{ $itemEtatRealisationMicroCompetence ? $itemEtatRealisationMicroCompetence->ordre : old('ordre') }}">
          @error('ordre')
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemEtatRealisationMicroCompetence" field="nom" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          ...
          @endif
          <label for="nom">
            {{ ucfirst(__('PkgApprentissage::etatRealisationMicroCompetence.nom')) }}
            <span class="text-danger">*</span>
          </label>
           <input
                name="nom"
                ...
                id="nom"
                placeholder="{{ __('PkgApprentissage::etatRealisationMicroCompetence.nom') }}"
                value="{{ $itemEtatRealisationMicroCompetence ? $itemEtatRealisationMicroCompetence->nom : old('nom') }}">
          @error('nom')
            <div class="text-danger">{{ $message }}</div>
          @enderror
      </div>
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemEtatRealisationMicroCompetence" field="code" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          ...
          @endif
          <label for="code">
            {{ ucfirst(__('PkgApprentissage::etatRealisationMicroCompetence.code')) }}
            <span class="text-danger">*</span>
          </label>
           <input
                name="code"
                ...
                id="code"
                placeholder="{{ __('PkgApprentissage::etatRealisationMicroCompetence.code') }}"
                value="{{ $itemEtatRealisationMicroCompetence ? $itemEtatRealisationMicroCompetence->code : old('code') }}">
          @error('code')
            <div class="text-danger">{{ $message }}</div>
          @enderror
      </div>
             ...
             <option value="">Sélectionnez une option</option>
                @foreach ($sysColors as $sysColor)
                    <option value="{{ $sysColor->id }}" data-color="{{ $sysColor->hex }}" 
                        {{ (isset($itemEtatRealisationMicroCompetence) && $itemEtatRealisationMicroCompetence->sys_color_id == $sysColor->id) || (old('sys_color_id>') == $sysColor->id) ? 'selected' : '' }}>
                        {{ $sysColor }}
                    </option>
                @endforeach
            </select>
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemEtatRealisationMicroCompetence" field="is_editable_only_by_formateur" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="is_editable_only_by_formateur" id="bulk_field_is_editable_only_by_formateur" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
          <label for="is_editable_only_by_formateur">
            {{ ucfirst(__('PkgApprentissage::etatRealisationMicroCompetence.is_editable_only_by_formateur')) }}
            ...
            <input
                ...
                value="1"
                {{ old('is_editable_only_by_formateur', $itemEtatRealisationMicroCompetence ? $itemEtatRealisationMicroCompetence->is_editable_only_by_formateur : 0) ? 'checked' : '' }}>
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemEtatRealisationMicroCompetence" field="description" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-12">
          ...
          <label for="description">
            {{ ucfirst(__('PkgApprentissage::etatRealisationMicroCompetence.description')) }}
    ...
    <div class="card-footer">
        ...
        <button type="submit" class="btn btn-info ml-2">{{ $itemEtatRealisationMicroCompetence->id ? __('Core::msg.edit') : __('Core::msg.add') }}</button>
    </div>
</form>
@show
...
<script>
    ...
    @if ($bulkEdit)
        window.modalTitle = '{{__("PkgApprentissage::etatRealisationMicroCompetence.singular") }} : {{__("Core::msg.edition_en_masse") }}'
    @else
