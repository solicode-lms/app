...
<form 
    ...
    @if ($itemRealisationChapitre->id)
        <input type="hidden" name="id" value="{{ $itemRealisationChapitre->id }}">
        @method('PUT')
    @endif
    ...
    <div class="row">
      ...
      @php $canEditchapitre_id = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'root')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'root')) ); @endphp
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="etat_realisation_chapitre_id" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="etat_realisation_chapitre_id" id="bulk_field_etat_realisation_chapitre_id" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="date_debut" :bulkEdit="$bulkEdit">
      @php $canEditdate_debut = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'root')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'root')) ); @endphp
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="date_fin" :bulkEdit="$bulkEdit">
      @php $canEditdate_fin = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'root')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'root')) ); @endphp
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="dernier_update" :bulkEdit="$bulkEdit">
      ...
      <div class="form-group col-12 col-md-6">
          ...
          <label for="dernier_update">
            {{ ucfirst(__('PkgApprentissage::realisationChapitre.dernier_update')) }}
                ...
                id="dernier_update"
                placeholder="{{ __('PkgApprentissage::realisationChapitre.dernier_update') }}"
                value="{{ $itemRealisationChapitre ? $itemRealisationChapitre->dernier_update : old('dernier_update') }}">
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="realisation_ua_id" :bulkEdit="$bulkEdit">
      @php $canEditrealisation_ua_id = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'root')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'root')) ); @endphp

      <div class="form-group col-12 col-md-6">
          @if ($bulkEdit)
          <div class="bulk-check">
              <input {{ $canEdittache_id ? '' : 'disabled' }} type="checkbox" class="check-input" name="fields_modifiables[]" value="realisation_ua_id" id="bulk_field_realisation_ua_id" title="Appliquer ce champ à tous les éléments sélectionnés" data-toggle="tooltip">
          </div>
          @endif
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="realisation_tache_id" :bulkEdit="$bulkEdit">
      @php $canEditrealisation_tache_id = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'root')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'root')) ); @endphp
...
<x-form-field :defined_vars="get_defined_vars()" :entity="$itemRealisationChapitre" field="commentaire_formateur" :bulkEdit="$bulkEdit">
      @php $canEditcommentaire_formateur = $bulkEdit ? Auth::user()->hasAnyRole(explode(',', 'formateur')) : (empty($itemRealisationChapitre->id) || Auth::user()->hasAnyRole(explode(',', 'formateur')) ); @endphp
