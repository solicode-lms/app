    ...
    <table class="table table-striped text-nowrap" style="table-layout: fixed; width: 100%;">
        <thead style="width: 100%">
            <tr>
                ...
                <x-sortable-column :sortable="true" width="35" field="module_id" modelname="realisationModule"
                    label="{!!ucfirst(__('PkgFormation::module.singular'))!!}" />
                <x-sortable-column :sortable="true" width="20" field="progression_cache" modelname="realisationModule"
                    label="{!!ucfirst(__('PkgApprentissage::realisationModule.progression_cache'))!!}" />
                <x-sortable-column :sortable="true" width="10" field="note_cache" modelname="realisationModule"
                    label="{!!ucfirst(__('PkgApprentissage::realisationModule.note_cache'))!!}" />
        ...
        <tbody>
            ...
            @foreach ($realisationModules_data as $realisationModule)
                ...
                <tr id="realisationModule-row-{{ $realisationModule->id }}" data-id="{{ $realisationModule->id }}"
                    @if ($hasMissingEvaluations) style="background-color: #ffe6e6;" @endif>
                    <x-checkbox-row :item="$realisationModule" :bulkEdit="$bulkEdit" />
                    <td style="max-width: 35%;white-space: normal;"
                        class="{{ $isEditable ? 'editable-cell' : '' }} text-truncate" data-id="{{$realisationModule->id}}"
                        ...
                        @if($realisationModules_permissions['edit-realisationModule'])
                            <x-action-button :entity="$realisationModule" actionName="edit">
                                @if($realisationModules_permissionsByItem['update'][$realisationModule->id])
                                    <a href="{{ route('realisationModules.edit', ['realisationModule' => $realisationModule->id]) }}"
                                        data-id="{{$realisationModule->id}}"
                                        class="btn btn-sm btn-default context-state editEntity">
                        ...
                        @if($realisationModules_permissions['show-realisationModule'])
                            <x-action-button :entity="$realisationModule" actionName="show">
                                @if($realisationModules_permissionsByItem['view'][$realisationModule->id])
                                    <a href="{{ route('realisationModules.show', ['realisationModule' => $realisationModule->id]) }}"
                                        data-id="{{$realisationModule->id}}"
                                        class="btn btn-default btn-sm context-state showEntity">
                        ...
                        <x-action-button :entity="$realisationModule" actionName="delete">
                            @if($realisationModules_permissions['destroy-realisationModule'])
                                @if($realisationModules_permissionsByItem['delete'][$realisationModule->id])
                                    <form class="context-state"
                                        action="{{ route('realisationModules.destroy', ['realisationModule' => $realisationModule->id]) }}"
                                        method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-default d-none d-lg-inline deleteEntity"
                                            data-id="{{$realisationModule->id}}">
