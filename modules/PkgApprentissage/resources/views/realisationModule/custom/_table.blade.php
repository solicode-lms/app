...
@extends('PkgApprentissage::realisationModule._table')
                ...
                <tr id="realisationModule-row-{{ $realisationModule->id }}" data-id="{{ $realisationModule->id }}"
                    ...
                    <td class="text-right wrappable" style="max-width: 15%;">
                        ...
                        @if($realisationModules_permissionsByItem['view'][$realisationModule->id])
                            @php
                                $showRoute = route('realisationModules.show', ['realisationModule' => $realisationModule->id]);
                                
                                if ($realisationModule->realisationCompetences->count() == 1) {
                                    $realisationCompetence = $realisationModule->realisationCompetences->first();
                                    
                                    if ($realisationCompetence->realisationMicroCompetences->count() == 1) {
                                        $realisationMicroCompetence = $realisationCompetence->realisationMicroCompetences->first();
                                        $showRoute = route('realisationMicroCompetences.show', ['realisationMicroCompetence' => $realisationMicroCompetence->id]);
                                    } else {
                                        $showRoute = route('realisationCompetences.show', ['realisationCompetence' => $realisationCompetence->id]);
                                    }
                                }
                            @endphp
                            <a href="{{ $showRoute }}" data-id="{{$realisationModule->id}}" class="btn btn-default btn-sm context-state showEntity">
