...
class BaseNatureLivrableController extends AdminController
{
    ...
    public function bulkUpdate(Request $request) {
        ...
        $request->validate([
            'natureLivrable_ids'   => ['required', 'array', 'min:1'],
            'fields_modifiables'               => ['required', 'array', 'min:1']
        ]);

        $ids          = $request->input('natureLivrable_ids', []);
        ...
        $form         = new \Modules\PkgCreationProjet\App\Requests\NatureLivrableRequest();
        ...
        foreach ($ids as $id) {
            $model = $this->natureLivrableService->find($id);
