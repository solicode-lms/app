            ...
            @foreach ($livrables_data as $livrable)
                <tr>
                    ...
                    <td class="text-center">
                        @can('show-livrable')
                            <a href="{{ route('livrables.show', array_merge($page, ['livrable' => $livrable->id])) }}" data-id="{{$livrable->id}}" class="btn btn-default btn-sm showEntity">
                                <i class="far fa-eye"></i>
                            </a>
                        @endcan
                        @can('edit-livrable')
                            <a href="{{ route('livrables.edit', array_merge($page, ['livrable' => $livrable->id])) }}" data-id="{{$livrable->id}}" class="btn btn-sm btn-default editEntity">
                                <i class="fas fa-pen-square"></i>
                            </a>
                        @endcan
                        @can('destroy-livrable')
                            <form action="{{ route('livrables.destroy',array_merge($page, ['livrable' => $livrable->id])) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
