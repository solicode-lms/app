...
class BaseEtatsRealisationProjetSeeder extends Seeder
{
    ...
    {
        $csvFile = fopen(base_path("modules/PkgRealisationProjets/Database/data/etatsRealisationProjets.csv"), "r");
        $firstline = true;

        while (($data = fgetcsv($csvFile)) !== false) {
            if (!$firstline) {
                EtatsRealisationProjet::create([
                    "titre" => $data[0] ,
                    "description" => $data[1] 
                ]);
            }
    ...
    private function assignPermissionsToRoles(string $AdminRole, string $MembreRole): void
    {
        ...
        $memberPermissions = Permission::whereIn('name', [
            'index-etatsRealisationProjet',
            'show-etatsRealisationProjet',
        ])->pluck('name')->toArray();
