...
class BaseLivrablesRealisationController extends AdminController
{
    ...
    public function bulkUpdate(Request $request) {
        ...
        $request->validate([
            'livrablesRealisation_ids'   => ['required', 'array', 'min:1'],
            'fields_modifiables'               => ['required', 'array', 'min:1']
        ]);

        $ids          = $request->input('livrablesRealisation_ids', []);
        ...
        $form         = new \Modules\PkgRealisationProjets\App\Requests\LivrablesRealisationRequest();
        ...
        foreach ($ids as $id) {
            $model = $this->livrablesRealisationService->find($id);
    ...
    {
        // $this->authorizeAction('update');
        $itemLivrablesRealisation = LivrablesRealisation::findOrFail($id);


        $data = $this->service->buildFieldMeta($itemLivrablesRealisation, $field);
        return response()->json(
            $data
        );
    }
    ...
    public function patchInline(Request $request, int $id)
    {
        ...
        $etag = $this->service->etag($itemLivrablesRealisation);
        ...
        $updated = $this->service->applyInlinePatch($itemLivrablesRealisation, $changes);
