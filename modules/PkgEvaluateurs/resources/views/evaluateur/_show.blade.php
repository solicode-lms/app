...
<div id="evaluateur-crud-show">
    ...
    @endif
            ...
            @if(
                  (auth()->user()?->can('show-evaluationRealisationProjet') && $itemEvaluateur->evaluationRealisationProjets->isNotEmpty())  
                  || auth()->user()?->can('create-evaluationRealisationProjet')
                  || (auth()->user()?->can('edit-evaluationRealisationProjet')  && $itemEvaluateur->evaluationRealisationProjets->isNotEmpty() )
                  )
            ...
            @if(
                  (auth()->user()?->can('show-evaluationRealisationTache') && $itemEvaluateur->evaluationRealisationTaches->isNotEmpty())  
                  || auth()->user()?->can('create-evaluationRealisationTache')
                  || (auth()->user()?->can('edit-evaluationRealisationTache')  && $itemEvaluateur->evaluationRealisationTaches->isNotEmpty() )
                  )
            <div class="col-12 col-md-6 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
...
<script>
    ...
    window.showUIId = 'evaluateur-crud-show';
