...
class BaseEtatEvaluationProjetImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatEvaluationProjet|null
     */
    private function findExistingRecord($reference): ?EtatEvaluationProjet
    {
        if($reference == null) return null;
        return EtatEvaluationProjet::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EtatEvaluationProjet([
             'ordre' => $values[0] ?? null,
             'code' => $values[1] ?? null,
             'titre' => $values[2] ?? null,
