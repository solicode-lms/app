    ...
    public function headings(): array
    {
        if ($this->format === 'csv') {
            return [
                ...
                'realisationUaPrototypes' => 'realisationUaPrototypes',
            ];
        } else {
            return [
                'reference' => __('Core::msg.reference'),
                'realisation_qcm_reference' => __('PkgQcm::realisationQcm.singular'),
                'question_reference' => __('PkgQcm::question.singular'),
                'date_reponse' => __('PkgQcm::reponseQcm.date_reponse'),
                    'propositionReponses' => __('PkgQcm::propositionReponse.plural'),
                    'realisationUaPrototypes' => __('PkgApprentissage::realisationUaPrototype.plural'),
            ];
        }
    }
    ...
    public function collection()
    {
        return $this->data->map(function ($reponseQcm) {
            return [
                ...
                'question_reference' => $reponseQcm->question?->reference,
                'date_reponse' => $reponseQcm->date_reponse,
                'propositionReponses' => $reponseQcm->propositionReponses
                    ->pluck('reference')
                    ->implode('|'),
                'realisationUaPrototypes' => $reponseQcm->realisationUaPrototypes
                    ->pluck('reference')
                    ->implode('|'),
            ];
        });
