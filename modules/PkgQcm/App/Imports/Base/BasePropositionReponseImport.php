...
class BasePropositionReponseImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return PropositionReponse|null
     */
    private function findExistingRecord($reference): ?PropositionReponse
    {
        if($reference == null) return null;
        return PropositionReponse::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new PropositionReponse([
             'ordre' => $values[0] ?? null,
             'reference' => $reference,
             'libelle' => $values[2] ?? null,
             'is_correcte' => $values[3] ?? null,
             'question_lib_id' => $values[4] ?? null,
             'reponseQcms' => $values[5] ?? null,
        ]);
