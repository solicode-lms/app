...
class BaseAffectationQcmProjetImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return AffectationQcmProjet|null
     */
    private function findExistingRecord($reference): ?AffectationQcmProjet
    {
        if($reference == null) return null;
        return AffectationQcmProjet::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new AffectationQcmProjet([
             'reference' => $reference,
