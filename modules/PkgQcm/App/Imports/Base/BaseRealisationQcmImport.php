...
namespace Modules\PkgQcm\App\Imports\Base;
...
class BaseRealisationQcmImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return RealisationQcm|null
     */
    private function findExistingRecord($reference): ?RealisationQcm
    {
        if($reference == null) return null;
        return RealisationQcm::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new RealisationQcm([
             'reference' => $reference,
             'affectation_qcm_projet_id' => $values[1] ?? null,
             'qcm_id' => $values[2] ?? null,
             'apprenant_id' => $values[3] ?? null,
             'etat_realisation_qcm_id' => $values[4] ?? null,
             'date_debut' => $values[5] ?? null,
             'date_fin' => $values[6] ?? null,
             'date_soumission' => $values[7] ?? null,
             'date_validation' => $values[8] ?? null,
             'note_obtenu' => $values[9] ?? null,
             'statut' => $values[10] ?? null,
        ]);
