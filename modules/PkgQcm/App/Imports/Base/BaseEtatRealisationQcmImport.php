...
class BaseEtatRealisationQcmImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return EtatRealisationQcm|null
     */
    private function findExistingRecord($reference): ?EtatRealisationQcm
    {
        if($reference == null) return null;
        return EtatRealisationQcm::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new EtatRealisationQcm([
             'reference' => $reference,
             'titre' => $values[1] ?? null,
             'description' => $values[2] ?? null,
             'is_editable_by_formateur' => $values[3] ?? null,
