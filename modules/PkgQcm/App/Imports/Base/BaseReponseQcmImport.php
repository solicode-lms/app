...
class BaseReponseQcmImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return ReponseQcm|null
     */
    private function findExistingRecord($reference): ?ReponseQcm
    {
        if($reference == null) return null;
        return ReponseQcm::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new ReponseQcm([
             'reference' => $reference,
             'realisation_qcm_id' => $values[1] ?? null,
             'question_qcm_id' => $values[2] ?? null,
             'date_reponse' => $values[3] ?? null,
             'propositionReponses' => $values[4] ?? null,
             'realisationUaProjets' => $values[5] ?? null,
        ]);
