...
class BaseQuestionQcmSeeder extends Seeder
{
    ...
    {
        $filePath = base_path("modules/PkgQcm/Database/data/questionQcms.csv");
        ...
        while (($data = fgetcsv($csvFile)) !== false) {
            ...
            if ($row) {
                ...
                if (!empty($row["question_lib_reference"])) {
                    $question_lib_id = \Modules\PkgQcm\Models\QuestionLib::where('reference', $row["question_lib_reference"])
                        ->value('id');
                }
                ...
                if (!empty($row["reference"])) {
                    $questionQcm = $questionQcmService->updateOrCreate(["reference" => $row["reference"]], $questionQcmData);
                } else {
                    $questionQcm = $questionQcmService->create($questionQcmData);
                }
            }
        }
    ...
    public function addDefaultControllerDomainFeatures(): void
    {
        ...
        $controllerName = 'QuestionQcmController';
        $controllerBaseName = 'questionQcm';
        $domainName = 'QuestionQcm';
