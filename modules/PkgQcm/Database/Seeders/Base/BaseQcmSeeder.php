...
class BaseQcmSeeder extends Seeder
{
    public static int $order = 103;
    ...
    {
        $filePath = base_path("modules/PkgQcm/Database/data/qcms.csv");
        ...
        while (($data = fgetcsv($csvFile)) !== false) {
            ...
            if ($row) {
                ...
                $qcmData =[
                        ...
                        "duree_minutes" => isset($row["duree_minutes"]) && $row["duree_minutes"] !== "" ? $row["duree_minutes"] : null,
                        "is_duree_limitee" => isset($row["is_duree_limitee"]) && $row["is_duree_limitee"] !== "" ? $row["is_duree_limitee"] : null,
                        "is_publie" => isset($row["is_publie"]) && $row["is_publie"] !== "" ? $row["is_publie"] : null,
                ...
                if (!empty($row["reference"])) {
                    $qcm = $qcmService->updateOrCreate(["reference" => $row["reference"]], $qcmData);
                } else {
                    $qcm = $qcmService->create($qcmData);
                }
            }
        }
    ...
    public function addDefaultControllerDomainFeatures(): void
    {
        ...
        $controllerName = 'QcmController';
        $controllerBaseName = 'qcm';
        $domainName = 'Qcm';
