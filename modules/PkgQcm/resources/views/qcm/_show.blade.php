...
<div id="qcm-crud-show">
    ...
    @endif
            ...
            @if(
                  (auth()->user()?->can('show-question') && $itemQcm->questions->isNotEmpty())  
                  || auth()->user()?->can('create-question')
                  || (auth()->user()?->can('edit-question')  && $itemQcm->questions->isNotEmpty() )
                  )
            <div class="col-12 col-md-12 mb-3 px-2 show-has-many">
                  <div class="border rounded p-2 h-100 " >
                  <small class="text-muted d-block">  {{ ucfirst(__('PkgQcm::question.plural')) }}</small>
                  <div class="pt-2">
                        @include('PkgQcm::question._index',['isMany' => true, "edit_has_many" => false,"contextKey" => 'qcm.show_' . $itemQcm->id])
                  </div>
                  </div>
            </div>
            @endif
