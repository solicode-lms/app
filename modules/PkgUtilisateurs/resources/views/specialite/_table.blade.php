            ...
            @foreach ($data as $specialite)
                <tr>
                    ...
                    <td class="text-center">
                        @can('show-SpecialiteController')
                            <a href="{{ route('specialites.show', $specialite) }}" class="btn btn-default btn-sm">
                                <i class="far fa-eye"></i>
                            </a>
                        @endcan
                        @can('edit-SpecialiteController')
                            <a href="{{ route('specialites.edit', $specialite) }}" class="btn btn-sm btn-default">
                                <i class="fas fa-pen-square"></i>
                            </a>
                        @endcan
                        @can('destroy-SpecialiteController')
                            <form action="{{ route('specialites.destroy', $specialite) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger"
                                    onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce specialite ?')">
...
<div class="d-md-flex justify-content-between align-items-center p-2">
    <div class="d-flex align-items-center mb-2 ml-2 mt-2">
        @can('import-SpecialiteController')
        ...
        @endcan
        @can('export-SpecialiteController')
            <form class="">
