...
class BaseDependanceTacheSeeder extends Seeder
{
    ...
    {
        $filePath = base_path("modules/PkgGestionTaches/Database/data/dependanceTaches.csv");
        ...
        while (($data = fgetcsv($csvFile)) !== false) {
            ...
            if ($row) {
                $dependanceTacheData =[
                    "tache_source_id" => $row["tache_source_id"] ?? null ,
                    "tache_cible_id" => $row["tache_cible_id"] ?? null ,
                    "type_dependance_tache_id" => $row["type_dependance_tache_id"] ?? null 
                ];
                if (!empty($row["reference"])) {
                    $dependanceTacheService->updateOrCreate(["reference" => $row["reference"]], $dependanceTacheData);
                } else {
                    $dependanceTacheService->create($dependanceTacheData);
                }
            }
        }
    ...
    private function addDefaultControllerDomainFeatures(): void
    {
        ...
        $controllerName = 'DependanceTacheController';
        $controllerBaseName = 'dependanceTache';
        $domainName = 'DependanceTache';
