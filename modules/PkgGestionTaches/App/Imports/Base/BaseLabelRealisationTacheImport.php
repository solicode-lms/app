...
class BaseLabelRealisationTacheImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return LabelRealisationTache|null
     */
    private function findExistingRecord($reference): ?LabelRealisationTache
    {
        if($reference == null) return null;
        return LabelRealisationTache::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new LabelRealisationTache([
             'nom' => $values[0] ?? null,
