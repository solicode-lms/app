...
class BaseTypeDependanceTacheImport implements ToModel, WithHeadingRow
{
    /**
     * Vérifie si un enregistrement avec la même référence existe.
     *
     * @param string $reference Référence unique de l'enregistrement.
     * @return TypeDependanceTache|null
     */
    private function findExistingRecord($reference): ?TypeDependanceTache
    {
        if($reference == null) return null;
        return TypeDependanceTache::where('reference', $reference)->first();
    }
    ...
    public function model(array $row)
    {
        ...
        return new TypeDependanceTache([
             'titre' => $values[0] ?? null,
