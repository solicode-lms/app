...
- **📝 Instructions d'Orchestration** :
  ...
  5. **Instructions de Suite** : Expliquer au développeur d'exécuter la migration (`php artisan migrate`) puis l'inviter à exécuter les commandes de création des interfaces CRUD par Gapp (`gapp meta:sync` puis `gapp make:crud [NomModel]`).
...
6. Dire au développeur de lancer la migration, puis `gapp make:crud`.
